<template>
  <div class="menu-featured__hero w-full">
    ...
    <div class="menu-featured__below-title w-full">
      ...
      <div class="menu-featured__items flex flex-col gap-3 sm:gap-3.5 lg:gap-4 w-full">
        <div
        ...
        >
          ...
          <span v-if="ramen.price" class="menu-featured__price shrink-0 tabular-nums text-lg sm:text-xl font-bold">
            {{ formatMenuPrice(ramen.price) }}
          </span>
        </div>

        <div v-if="ramen.addOnName" class="flex items-start justify-between gap-4 md:gap-6">
          <span class="menu-item-name text-lg sm:text-xl font-bold flex-1 leading-snug">{{ ramen.addOnName }}</span>
          <span v-if="ramen.addOnPrice" class="menu-featured__price shrink-0 tabular-nums text-lg sm:text-xl font-bold">
...
.menu-featured__below-title {
  margin-top: calc(-1 * var(--menu-featured-body-pull, 52px));
}
